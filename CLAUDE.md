...
- **`main`** — RÖRS INTE. `main` är inaktuell och kör i stort sett en helt annan kod idag. **Allt körs på `dev`.** Fråga ALDRIG om merge/deploy till `main` och föreslå det inte — det är inte aktuellt på lång tid.
- All kod ligger i **repo-roten** (ingen undermapp).
- Committa med korta beskrivande meddelanden på svenska eller engelska. Pusha alltid till `dev` — aldrig `main`.
...
Kör `node --check` på alla `.js` i roten och `overlay/` samt på varje inline `<script>`-block i HTML-filerna (med radnummer som pekar rätt i HTML-filen), letar nya nästlade template literals, verifierar att `server.js` och `server-lanstrat.js` är byte-identiska, och kontrollerar `tv.html`s skärmväxling (`function show(id)`) — att högst en skärm-div saknar `display:none` som standard, så inte flera skärmar syns samtidigt vid sidladdning innan första `show()`-anropet (hänt med `#studio-screen`, fixat 2026-07-27). Exit 0 = grönt.
...
- **Lokal körning:** `server.js` läser aldrig `.env` själv (inget `dotenv`-anrop) — den förlitar sig på att Railway sätter miljövariablerna på plattformsnivå. Lokalt måste `.env` laddas in i skalmiljön manuellt innan `node server.js` körs, annars kraschar servern vid start (`initBins` försöker skapa nya JSONBin-bins istället för att läsa befintliga). **Använd INTE** `set -a; source .env; set +a` — trasigt på två sätt, bekräftat 2026-07-13: värden med `$` (t.ex. JSONBins `$2b$10$...`-nyckelformat) blir variabelexpanderade till skräp, och sista raden i filen hoppas tyst över om den saknar avslutande radbrytning (vilket `.env` gjorde). Använd istället en rad-för-rad-loop som inte exekverar filen som skript:
  ```bash
  set -a
