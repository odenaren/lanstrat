...
async function generateForMatch(strategyMatch, heroes, allMatches) {
  ...
  (m.teamfights || []).forEach(tf => {
    ...
    (tf.players || []).forEach((pl, idx) => {
      ...
      if (isUs) { dhsG += (pl.gold_delta||0); const nick = aliasByHero[normHero(heroName(op.hero_id))]; dhsPlayers.push(nick || heroName(op.hero_id)); }
  ...
  for (let i = 0; i < recap.dialogue.length; i++) {
    ...
    if (!tRes.ok) {
      ...
      console.error('  ElevenLabs ' + tRes.status + ' pa replik ' + i + ': ' + detail + ' — avbryter denna match.');
      return;
    }
